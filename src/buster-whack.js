...
export function mountBusterWhack(container, options = {}) {
  ...
  function showSplash() {
    showOverlay({
      eyebrow: "virus busting",
      title: "BUSTER WHACK",
      sub:
        "move: pad, WASD or arrows — fire: tap, hold to charge\n" +
        "yellow mett: tap it · steel: charged shots only\n" +
        "green hopper: 2 taps or 1 charge\n" +
        "cyan prog: hold fire — safe until fully up\n" +
        "gold: jackpot, be quick · P pause · M mute",
      stats: S.best ? statRows([["best score", S.best, "big"]]) : "",
      buttons: [{ label: "START", fn: () => { audioCtx(); resetGame(S.clock); } }],
    });
  }
