...
function place(s, o) {
  const e = {
    ...
    state: "up", t0: s.clock, riseMs: C.RISE_MS, hp: o.hp === undefined ? 1 : o.hp,
...
test("in advance the level is the arena you are in; classic still counts kills", () => {
  ...
  c.deletions = 30;
  assert.equal(hudView(c).level, C.level(30));
});

test("the road ramps for a hundred arenas and is still climbing at the end", () => {
  assert.deepEqual(C.arenaPlan(0), { pool: 4, waveSize: 2 });
  const p50 = C.arenaPlan(50), p99 = C.arenaPlan(99), p100 = C.arenaPlan(100);
  assert.ok(p50.pool > C.arenaPlan(10).pool, "still growing at 50");
  assert.ok(p99.pool >= p50.pool && p100.pool >= p99.pool);
  assert.ok(p100.waveSize > C.arenaPlan(0).waveSize);
  assert.ok(C.arenaPlan(500).waveSize <= 5, "never more than the board can hold");
});

test("unlocks are keyed to arenas, with the card shown as you step in", () => {
  const s = advanceGame();
  // before the guards' arena: no guards in the plan
  jumpToArena(s, C.ADV_UNLOCK.guard - 1, false);
  const a = activeArena(s.world);
  ...
  jumpToArena(s, C.ADV_UNLOCK.guard, false);
...
test("entering arena 100 flips the run to UNLIMITED", () => {
  const s = advanceGame();
  jumpToArena(s, C.ROAD_END, false);
  const a = activeArena(s.world);
  a.entered = false; s.player.col = a.x0 - 1;
  step(s, 16, []);
  ...
  assert.equal(hudView(s).level, C.ROAD_END + 1);
});
...
test("a bomb arcs three columns, splashes a 3x3 at charged strength, and can hit you", () => {
  ...
  step(s2, C.BOMB_ARC_MS + 20, []);
  assert.ok(s2.timeLeft < t0 - C.HIT_TIME_PENALTY + 0.2, "your own bomb costs you the hit penalty");
});
...
test("the spawner actually deals Sentinels at their arena, with fields from the table", () => {
  ...
  jumpToArena(s, C.ADV_UNLOCK.sentinel2 + 2);
  const a = activeArena(s.world);
